...
CLUSTERS = [
    ...
    },
    {
        ...
        "reports":  [3, 4, 7, 8, 18, 20, 22, 24, 38, 40],  # products, categories, ratings
    ...
    },
    {
        ...
        "reports":  [9, 14, 15, 16, 17, 25, 27, 34, 39],  # KPIs, subscriptions, discounts
...
def random_name():
    firstnames = [
        ...
        "Yara", "Zoe", "Adam", "Bella", "Carlos", "Daisy"
    ]
    lastnames = [
...
def pick_report(cluster: dict) -> tuple:
    ...
    model   = random.choices(
        list(cluster["models"].keys()),
        weights=list(cluster["models"].values())
    )[0]
    context = random.choices(
        list(cluster["contexts"].keys()),
        weights=list(cluster["contexts"].values())
    )[0]
...
def insert_recent_view(conn, user_id: int, model: str, model_id: int, context: str, ts: datetime):
    with conn.cursor() as cur:
        cur.execute("""
...
def run():
    ...
    for cluster_idx, cluster in enumerate(CLUSTERS):
        ...
        count = USERS_PER_CLUSTER if cluster_idx < 5 else (TOTAL_USERS - USERS_PER_CLUSTER * 5)
    ...
    print("Now reset publisher cursors and restart the pipeline to sync into your tables.")
